...
def get_condconv_initializer(initializer, num_experts, expert_shape):
    ...
    def condconv_initializer(expected_shape, dtype=None):
...
class CondConv2D(tf.keras.layers.Conv2D):
    ...
    Attributes:
      ...
      num_experts: The number of expert kernels and biases in the CondConv
      layer.
    ...
    def __init__(self, filters, kernel_size, num_experts, strides=(1, 1),
            padding='valid', data_format=None, dilation_rate=(1, 1),
            activation=None, use_bias=True, kernel_initializer='glorot_uniform',
            bias_initializer='zeros', kernel_regularizer=None,
            bias_regularizer=None, activity_regularizer=None,
            kernel_constraint=None, bias_constraint=None, **kwargs):
        super(CondConv2D, self).__init__(filters=filters,
    ...
    def build(self, input_shape):
        if len(input_shape) != 4:
            raise ValueError('Inputs to `CondConv2D` should have rank 4. '
        ...
        self.condconv_kernel = self.add_weight(name='condconv_kernel',
            shape=condconv_kernel_shape,
            initializer=get_condconv_initializer(
        ...
        if self.use_bias:
            ...
            self.condconv_bias = self.add_weight(name='condconv_bias',
    ...
    def call(self, inputs, routing_weights):
        ...
        for input_tensor, kernel in zip(inputs, kernels):
            ...
            outputs_list.append(
                tf.nn.convolution(input_tensor, kernel, strides=self.strides,
        ...
        if self.use_bias:
            ...
            for output, bias in zip(outputs, biases):
                ...
                bias_outputs_list.append(tf.nn.bias_add(output, bias,
                    data_format=self.converted_data_format))
