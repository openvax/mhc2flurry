...
from .allele_encoding_pair import AlleleEncodingPair
...
class Class2NeuralNetwork(object):
    """
    Low level class II predictor consisting of a single neural network.
    
    Both single allele and pan-allele prediction are supported.
    
    Users will generally use Class2AffinityPredictor, which gives a higher-level
    ...
    def fit(
        ...
        allele_encoding : AlleleEncodingPair
        ...
        random_negatives_planner.plan(
            ...
            alleles=(
                allele_encoding_pair.allele_pairs if allele_encoding_pair
                else None
            ),
            inequalities=inequalities)

        random_negatives_allele_encoding = None
        if allele_encoding_pair is not None:
            random_negatives_allele_encoding = allele_encoding_pair.from_pairs(
        ...
        if allele_encoding_pair is not None:
            (alpha_allele_encoding_input, alpha_allele_representations) = (
                self.allele_encoding_to_network_input(
                    allele_encoding_pair.alpha_allele_encoding))
            (beta_allele_encoding_input, beta_allele_representations) = (
                self.allele_encoding_to_network_input(
                    allele_encoding_pair.beta_allele_encoding))
            x_dict_without_random_negatives[
                'alpha_allele'
            ] = alpha_allele_encoding_input
            x_dict_without_random_negatives[
                'beta_allele'
            ] = beta_allele_encoding_input
        ...
        if self.network() is None:
            self._network = self.make_network(
                alpha_allele_representations=alpha_allele_representations,
                beta_allele_representations=beta_allele_representations,
                **self.network_hyperparameter_defaults.subselect(
                    self.hyperparameters))
            if verbose > 0:
                self.network().summary()
        ...
        if alpha_allele_representations is not None:
            self.set_allele_representations(
                alpha_allele_representations, beta_allele_representations)
        ...
        for i in range(self.hyperparameters['max_epochs']):
            ...
            if not x_dict_with_random_negatives:
                if len(random_negative_peptides) > 0:
                    ...
                    if 'alpha_allele' in x_dict_without_random_negatives:
                        x_dict_with_random_negatives[
                            'alpha_allele'
                        ] = numpy.concatenate([
                            self.allele_encoding_to_network_input(
                                random_negatives_allele_encoding.alpha_allele_encoding
                            )[0],
                            x_dict_without_random_negatives['alpha_allele']
                        ])
                        x_dict_with_random_negatives[
                            'beta_allele'
                        ] = numpy.concatenate([
                            self.allele_encoding_to_network_input(
                                random_negatives_allele_encoding.beta_allele_encoding
                            )[0],
                            x_dict_without_random_negatives['beta_allele']
                        ])
    ...
    def predict(
        ...
        if allele_encoding_pair is not None:
            ...
            x_dict['alpha_allele'] = alpha_allele_encoding_input
            x_dict['beta_allele'] = beta_allele_encoding_input
            self.set_allele_representations(
                alpha_allele_representations, beta_allele_representations)
            network = self.network()
        else:
    ...
    def merge(cls, models, merge_method="average"):
        ...
        Parameters
        ----------
        models : list of Class2NeuralNetwork
            instances to merge
        ...
        result = Class2NeuralNetwork(**dict(models[0].hyperparameters))
    ...
    def make_network(
            ...
            allele_dense_layer_sizes,
            allele_positionwise_embedding_size,
            layer_sizes,
        ...
        # Apply first peptide convolution
        peptide_convolutions = list(peptide_convolutions)
        first_peptide_convolution = peptide_convolutions.pop(0)
        ...
        if alpha_allele_representations is not None:
            ...
            alpha_allele_input = Input(
                shape=(1,),
                dtype='float32',
                name='alpha_allele')
            inputs.append(alpha_allele_input)

            alpha_allele_layer = Embedding(
                name="alpha_allele_representation",
                input_dim=alpha_allele_representations.shape[0],
                output_dim=numpy.product(
                    alpha_allele_representations.shape[1:], dtype=int),
                input_length=1,
                trainable=False)(alpha_allele_input)
            alpha_allele_layer = keras.layers.Reshape(
                alpha_allele_representations.shape[1:])(alpha_allele_layer)

            beta_allele_input = Input(
                shape=(1,),
                dtype='float32',
                name='beta_allele')
            inputs.append(beta_allele_input)

            beta_allele_layer = Embedding(
                name="beta_allele_representation",
                input_dim=beta_allele_representations.shape[0],
                output_dim=numpy.product(
                    beta_allele_representations.shape[1:], dtype=int),
                input_length=1,
                trainable=False)(beta_allele_input)
            beta_allele_layer = keras.layers.Reshape(
                beta_allele_representations.shape[1:])(beta_allele_layer)

            allele_layer = keras.layers.concatenate(
                [alpha_allele_layer, beta_allele_layer],
                axis=1,
                name="alpha_beta_representation_merged")

            allele_layer = keras.layers.LocallyConnected1D(
                name="allele_lc",
                filters=allele_positionwise_embedding_size,
                kernel_size=1,
                activation="relu",
                kernel_regularizer=kernel_regularizer,
            )(allele_layer)

            allele_layer = keras.layers.GlobalAveragePooling1D(
                name="allele_lc_pooled")(allele_layer)
            ...
            current_layer = keras.layers.Multiply(
                name="peptide_allele_merge",
            )([current_layer, allele_layer])

            """

            # Expand dims of current_layer so a 2d Conv can work. This is just
            # becauase we have an implementation of CondConv2d, not CondConv1d.
            # This is also why we adjust kernel_size to have height 1.
            # Apply conditional convolution
            # Reshape to height, width, channels. Height is 1.
            from tensorflow.keras.backend import int_shape
            current_shape = int_shape(current_layer)
            print(current_shape)
            current_layer = keras.layers.Reshape(
                (1, current_shape[1], current_shape[2]))(current_layer)

            kwargs = dict(peptide_convolution)
            kwargs['kernel_size'] = (1, kwargs['kernel_size'])  # height is 1
            
            current_layer = condconv.CondConv2D(
                num_experts=allele_convolution_bank_size,
                padding="same",
                data_format="channels_last",  # NHWC
                **kwargs)(current_layer, convolution_bank_weights)
            """

        # Apply remaining peptide convolutions
        for (i, peptide_convolution) in enumerate(peptide_convolutions):
            current_layer = keras.layers.Conv1D(
                name="peptide_additional_conv_%d" % i,
                padding="same",
                **peptide_convolution)(current_layer)

        # Global max pooling after all peptide convolutions.
        current_layer = keras.layers.GlobalMaxPooling1D()(current_layer)
    ...
    def clear_allele_representations(self):
        ...
        layer_names = [
            "alpha_allele_representation", "beta_allele_representation"
        ]

        target_representations = []
        for name in layer_names:
            layer = original_model.get_layer(name)
    ...
    def set_allele_representations(
            self,
            alpha_allele_representations,
            beta_allele_representations,
            force_surgery=False):
        """
        ...
        names_and_targets = [
            ("alpha_allele_representation", alpha_allele_representations),
            ("beta_allele_representation", beta_allele_representations),
        ]

        for (name, allele_representations) in names_and_targets:
            ...
            if existing_weights_shape != reshaped.shape or force_surgery:
                ...
                layer = new_model.get_layer(name)
